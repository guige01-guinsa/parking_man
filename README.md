...
기본 아파트 코드는 `APT1100`입니다. 첫 로그인 후 관리자 화면에서 현장 담당자 계정을 추가하고, 권한 변경 및 비밀번호 재설정을 할 수 있습니다.

## 여러 아파트 운영

- 로그인 시 `아파트 코드 + 아이디 + 비밀번호`를 입력합니다.
- 차량, Excel 동기화 이력, OCR 학습, 단속 기록, CCTV 검색요청은 아파트 코드별로 분리됩니다.
- 같은 아이디도 아파트가 다르면 별도 계정으로 사용할 수 있습니다.
- 관리자 화면의 `아파트 등록 및 현황`에서 새 아파트 코드와 초기 관리자 계정을 만들 수 있습니다.
